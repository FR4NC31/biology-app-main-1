...
const Home = () => {
  ...
  const handleLogout = () => {
    ...
    setLoggedInUser('');
    setUsername('');
    setShowModal(true); // Show modal again after logout
  };
  ...
  return (
    <div className="min-h-screen min-w-screen bg-gradient-to-b from-green-100 to-white flex flex-col items-center justify-center p-4 relative">
      ...
      {/* Login/Logout Toggle */}
      <button
        onClick={loggedInUser ? handleLogout : () => setShowModal(true)}
      ...
      <div className="bg-white shadow-2xl rounded-2xl p-8 max-w-xl w-full">
        ...
        <ul className="space-y-4">
          ...
          <li>
            <Link
              to="/leaderboard"
              className="flex items-center gap-2 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 px-4 py-3 rounded-lg font-medium transition duration-200"
            >
